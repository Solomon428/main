...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const stats = {
      totalOverdue: total,
      totalAmount: invoicesWithDetails.reduce((sum, inv) => sum + inv.totalAmount, 0),
      totalWithPenalty: invoicesWithDetails.reduce((sum, inv) => sum + inv.totalWithPenalty, 0),
      criticalCount: invoicesWithDetails.filter((inv) => inv.daysOverdue > 30).length,
    };
    ...
    switch (action) {
      case 'mark_paid':
        updateData = {
          ...
          amountPaid: { set: prisma.$queryRaw`SELECT totalAmount FROM invoices WHERE id = ${invoiceId}` },
          amountDue: 0,
        };
        break;
      case 'update_penalty':
