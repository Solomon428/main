...
if (!JWT_SECRET) {
  console.error('[Login API] CRITICAL: JWT_SECRET environment variable is not set!');
  console.error('[Login API] Please check your .env file has JWT_SECRET configured');
}

const FALLBACK_SECRET = 'creditorflow-secure-jwt-secret-key-for-production-environment-min-32-chars';
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!validation.success) {
      console.error('[Login API] Validation failed:', validation.error.format());
      return NextResponse.json(
    ...
    if (!JWT_SECRET) {
      console.error('[Login API] Cannot generate token: JWT_SECRET not configured');
      return NextResponse.json(
    ...
    }).catch(err => console.error('[Login API] Failed to update lastLoginAt:', err));
