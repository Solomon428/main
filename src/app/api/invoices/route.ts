...
async function handleFileUpload(request: NextRequest): Promise<NextResponse> {
  ...
  const invoice = await prisma.invoices.create({
    data: {
      invoiceNumber: manualData.invoiceNumber || extractedData?.invoiceNumber || `TEMP-${timestamp}`,
      supplierId: manualData.supplierId || supplierId || null,
      supplierName: manualData.supplierName || extractedData?.supplierName || 'Unknown Supplier',
      ...
      amountDue: manualData.totalAmount || extractedData?.amountDue || extractedData?.totalAmount || 0,
