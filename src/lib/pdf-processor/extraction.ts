...
export async function analyzeDocumentQuality(
  ...
  try {
    if (mimeType === "application/pdf") {
      ...
      return {
        clarityScore: hasText ? 0.95 : hasImages ? 0.6 : 0.3,
...
export function determineExtractionMethod(
  ...
  if (
    ...
    qualityMetrics.isSearchable &&
    qualityMetrics.clarityScore > 0.8
  ) {
    return "native-pdf";
  }
  ...
  if (mimeType === "application/pdf" || mimeType.startsWith("image/")) {
    const preferredEngine =
      processingOptions?.preferredOCREngine || OCR_ENGINES.TESSERACT;
...
export async function performExtraction(
  ...
  try {
    switch (extractionMethod) {
      ...
      case "ocr-tesseract":
        return await extractWithTesseractOCR(
          filePath,
          processingId,
          processingOptions,
        );
      ...
      case "ocr-azure":
        return await extractWithAzureOCR(
          filePath,
          processingId,
          processingOptions,
        );
      ...
      case "ocr-google":
        return await extractWithGoogleOCR(
          filePath,
          processingId,
          processingOptions,
        );
      ...
      case "ocr-amazon":
        return await extractWithAmazonOCR(
          filePath,
          processingId,
          processingOptions,
        );
      ...
      case "ocr-ollama":
        return await extractWithOllamaOCR(
          filePath,
          processingId,
          processingOptions,
        );
...
export async function extractWithNativePDF(
  ...
  try {
    ...
    const textMetrics: TextExtractionMetrics = {
      ...
      averageWordLength:
        rawText.split(/\s+/).reduce((sum, word) => sum + word.length, 0) /
        Math.max(rawText.split(/\s+/).length, 1),
      averageLineLength:
        rawText.split("\n").reduce((sum, line) => sum + line.length, 0) /
        Math.max(rawText.split("\n").length, 1),
    ...
    const tableMetrics: TableExtractionMetrics = {
      ...
      totalCells: tables.reduce(
        (sum, table) =>
          sum +
          table.rows.reduce(
            (rowSum: number, row: any) => rowSum + row.cells.length,
            0,
          ),
        0,
      ),
      extractionConfidence: tables.length > 0 ? 0.85 : 0.3,
    ...
    const formMetrics: FormExtractionMetrics = {
      ...
      totalValues: fields.filter((f: any) => f.fieldValue.trim().length > 0)
        .length,
      extractionConfidence: fields.length > 0 ? 0.8 : 0.4,
      fieldDetectionConfidence: 0.75,
      valueExtractionConfidence: 0.7,
    };
