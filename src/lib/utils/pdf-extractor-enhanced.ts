...
// Local fallback type to avoid dependency on a missing ExtractedInvoiceData export
type ExtractedInvoiceData = any;
// Removed circular dependency: enhanced path no longer imports the canonical extractor
...
export class EnhancedExtractor {
  ...
  static async extractInvoiceData(filePath: string): Promise<EnhancedExtractionResult> {
    ...
    try {
      // Baseline text extraction without importing the canonical extractor
      // Note: circular dependency removed on purpose; provide a safe fallback instead
  ...
  private static async extractWithOCR(filePath: string): Promise<EnhancedExtractionResult> {
    ...
    try {
      ...
      if (filePath.match(/\.(png|jpg|jpeg)$/i)) {
        ...
        // Fallback to basic extraction from OCR text (no canonical extractor import available)
