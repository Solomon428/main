...
export class SARSValidator {
   ...
   */
  static async validateSanctions(
    invoiceId: string,
    ...
    if (invoice.supplierId) {
      ...
      if (supplier?.country) {
        ...
        if (this.SANCTIONED_COUNTRIES.includes(countryCode)) {
          violations.push(
            `Supplier country ${supplier.country} (${countryCode}) is sanctioned`,
          );
        }
      }
    }
    ...
    const riskLevel =
      violations.length > 1 ? "HIGH" : violations.length > 0 ? "MEDIUM" : "LOW";
  ...
  static async validatePEP(invoiceId: string): Promise<PEPValidationResult> {
    ...
    const riskLevel =
      matches.length > 2 ? "HIGH" : matches.length > 0 ? "MEDIUM" : "LOW";
