 ...
 */
export function normalizeVATNumber(vatNumber: string | undefined): string | undefined {
 ...
 */
export function formatVATNumberWithPrefix(vatNumber: string, countryCode: string): string {
  const normalized = normalizeVATNumber(vatNumber);
  if (!normalized) return vatNumber;
 ...
 */
export function extractCountryCode(vatNumber: string): { countryCode: string; number: string } {
  const normalized = normalizeVATNumber(vatNumber) || '';
  
  // Check for common EU country codes at the start
  const euCountryCodes = ['AT', 'BE', 'BG', 'HR', 'CY', 'CZ', 'DK', 'EE', 'FI', 'FR', 
 ...
 */
export function isValidVATFormat(vatNumber: string | undefined, countryCode: string): boolean {
  ...
  switch (upperCountryCode) {
    ...
    case 'UK':
      ...
      return /^([0-9]{9}([0-9]{3})?|[A-Z]{2}[0-9]{3})$/.test(normalized) ||
