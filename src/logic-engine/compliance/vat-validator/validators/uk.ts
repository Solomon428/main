 ...
 */
export const UK_STANDARD_VAT_RATE = 0.20; // 20%
...
export function validateUKVATNumber(
  ...
  // Check patterns
  const isValidFormat = Object.values(UK_VAT_PATTERNS).some(pattern => pattern.test(normalized));
  ...
  return {
    ...
    warnings: isNorthernIreland ? [{
      ...
      warningMessage: 'Northern Ireland VAT number detected - EU VAT rules may apply',
      severity: 'INFO',
      timestamp: new Date()
    }] : [{
      field: 'supplierVATNumber',
      warningCode: 'UK_VAT_VALIDATE_HMRC',
      warningMessage: 'Consider validating via HMRC API for real-time verification',
      severity: 'INFO',
      timestamp: new Date()
    }],
    metadata: { validationId, isNorthernIreland }
...
export function validateUKTaxInvoice(
  ...
  if (!input.supplierVATNumber) {
    ...
    const vatResult = validateUKVATNumber(input.supplierVATNumber, validationId);
