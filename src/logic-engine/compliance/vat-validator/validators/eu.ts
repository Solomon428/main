 ...
 */
export const EU_STANDARD_VAT_RATE = 0.20; // 20% - varies by country

/**
 * EU VAT rates by country
 */
export const EU_VAT_RATES: Record<string, { standard: number; reduced?: number; superReduced?: number }> = {
  AT: { standard: 0.20, reduced: 0.10, superReduced: 0.00 },
  BE: { standard: 0.21, reduced: 0.06 },
  BG: { standard: 0.20, reduced: 0.09 },
  HR: { standard: 0.25, reduced: 0.13 },
  CY: { standard: 0.19, reduced: 0.09 },
  CZ: { standard: 0.21, reduced: 0.15, superReduced: 0.10 },
  DK: { standard: 0.25 },
  EE: { standard: 0.20, reduced: 0.09 },
  FI: { standard: 0.24, reduced: 0.14, superReduced: 0.10 },
...
export function validateEUVATNumber(
  ...
  return {
    ...
    warnings: [{
      ...
      warningMessage: 'Consider validating via VIES for real-time verification',
      severity: 'INFO',
      timestamp: new Date()
    }],
    metadata: { validationId, countryCode }
...
export function validateEUTaxInvoice(
  ...
  if (!input.supplierVATNumber) {
    errors.push({
      ...
      errorMessage: 'Supplier VAT number is mandatory for intra-EU transactions',
      severity: 'ERROR',
      timestamp: new Date()
    });
    score -= 20;
