...
export async function validateSARSVATNumber(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`SARS API error: ${response.status} ${response.statusText}`);
    }
...
export async function validateEUVATNumber(
  ...
  try {
    ...
    return {
      ...
      vatNumber: normalizedNumber,
      normalizedNumber,
      errors: [`VIES validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`],
      warnings: ['Falling back to offline validation']
    };
  }
}
...
export async function validateUKVATNumber(
  ...
  try {
    // Note: This is a placeholder implementation
    const response = await fetch(`${config.baseUrl}/check-vat-number/${number}`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${config.apiKey}`,
    ...
    if (!response.ok) {
      throw new Error(`HMRC API error: ${response.status} ${response.statusText}`);
    }
    ...
    return {
      ...
      vatNumber: normalizedNumber,
      normalizedNumber,
      errors: [`HMRC validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`],
      warnings: ['Falling back to offline validation']
    };
  }
}
  ...
  switch (upperCountryCode) {
    ...
    default:
      return {
        ...
        errors: [`API validation not supported for country: ${upperCountryCode}`],
