...
export class InvoiceComplianceChecker {
   ...
   */
  static async checkInvoiceCompliance(invoiceId: string): Promise<ComplianceCheckResult> {
    ...
    try {
      const supplier = invoice.supplierId 
        ? await prisma.suppliers.findUnique({ where: { id: invoice.supplierId } })
        : null;
      ...
      const fraudInput = {
        ...
        hasDuplicatePattern: checks.find(c => c.name === 'Duplicate Check')?.passed === false,
        hasVATAnomaly: checks.find(c => c.name === 'VAT Validation')?.passed === false,
      ...
      checks.push({
        ...
        severity: fraudResult.riskLevel === 'HIGH' ? 'HIGH' : fraudResult.riskLevel === 'MEDIUM' ? 'MEDIUM' : 'LOW',
      });
    } catch (error) {
      checks.push({
        name: 'Fraud Scoring',
    ...
    const highSeverityChecks = checks.filter(c => !c.passed && c.severity === 'HIGH');
    const isCompliant = failedChecks.length === 0;
    const overallRiskLevel = highSeverityChecks.length > 0 ? 'HIGH' :
                            failedChecks.length > 2 ? 'MEDIUM' : 'LOW';
    const requiresManualReview = highSeverityChecks.length > 0 || failedChecks.length > 2;
    ...
    await prisma.invoices.update({
      ...
      data: {
        ...
        status: isCompliant ? InvoiceStatus.PENDING_APPROVAL : InvoiceStatus.UNDER_VALIDATION,
      },
    });
    ...
    await auditLogger.log({
      ...
      metadata: {
        checks: checks.map(c => ({ name: c.name, passed: c.passed, severity: c.severity })),
        isCompliant,
        overallRiskLevel,
