...
export async function detectFuzzyMatches(
  ...
  const invoiceNumberMatches = await fuzzyMatchInvoiceNumber(input, context, checkId);
  matches.push(...invoiceNumberMatches);
  
  // Supplier name fuzzy matching
  const supplierNameMatches = await fuzzyMatchSupplierName(input, context, checkId);
...
export async function fuzzyMatchInvoiceNumber(
  ...
  for (const historical of context.historicalInvoices) {
    ...
    const maxLength = Math.max(invoiceNumber.length, historical.invoiceNumber.length);
    ...
    const bestSimilarity = Math.max(levenshteinSimilarity, jaroWinklerSimilarity);
    ...
    if (bestSimilarity >= FUZZY_MATCH_SETTINGS.JARO_WINKLER_THRESHOLD) {
      matches.push({
        ...
        algorithm: jaroWinklerSimilarity > levenshteinSimilarity ? 'JARO_WINKLER' : 'LEVENSHTEIN',
...
export async function fuzzyMatchSupplierName(
  ...
  for (const historical of context.historicalInvoices) {
    ...
    if (soundexMatch && metaphoneMatch) {
      confidence = Math.max(confidence, FUZZY_MATCH_SETTINGS.METAPHONE_SIMILARITY_THRESHOLD);
      algorithm = 'METAPHONE';
    } else if (soundexMatch) {
      confidence = Math.max(confidence, FUZZY_MATCH_SETTINGS.SOUNDEX_SIMILARITY_THRESHOLD);
      algorithm = 'SOUNDEX';
    }
...
export async function matchAmount(
  ...
  for (const historical of context.historicalInvoices) {
    ...
    const isMatch = amountDifference <= absoluteTolerance || 
                   percentageDifference <= percentageTolerance;
    
    if (isMatch) {
      // Calculate confidence based on how close the amounts are
      const confidence = 1 - Math.min(percentageDifference / percentageTolerance, 1);
