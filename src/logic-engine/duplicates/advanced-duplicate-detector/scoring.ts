...
export function calculateConfidence(
  ...
  return {
    ...
    requiresManualReview:
      overallConfidence >= DUPLICATE_THRESHOLDS.LOW_CONFIDENCE &&
      overallConfidence < DUPLICATE_THRESHOLDS.HIGH_CONFIDENCE,
    isDefinitiveMatch:
      overallConfidence >= DUPLICATE_THRESHOLDS.VERY_HIGH_CONFIDENCE,
    isDefinitiveNonMatch:
      overallConfidence < DUPLICATE_THRESHOLDS.MIN_CONFIDENCE,
  };
}
...
export function determineDuplicateType(
  ...
  if (confidenceResult.isDefinitiveMatch) {
    ...
    if (
      fuzzyMatches.some(
        (m) => m.matchType === "INVOICE_NUMBER" && m.confidence >= 0.95,
      )
    ) {
      return "EXACT";
    }
    ...
    if (
      fuzzyMatches.some(
        (m) => m.matchType === "INVOICE_NUMBER" && m.confidence >= 0.85,
      )
    ) {
      return "FUZZY";
    }
  ...
  fuzzyMatches.forEach((match) => {
    duplicates.push({
      ...
      requiresInvestigation:
        ...
        riskLevel === "CRITICAL" ||
        riskLevel === "SEVERE",
      investigationPriority: determineInvestigationPriority(
        riskLevel,
        match.confidence,
        checkId,
      ),
...
export function determineInvestigationPriority(
  ...
  if (riskLevel === "HIGH" && confidence >= 0.9) return "URGENT";
  if (riskLevel === "HIGH") return "HIGH";
  if (riskLevel === "MEDIUM" && confidence >= 0.8) return "MEDIUM";
  return "LOW";
}
...
export function identifyDuplicatePatterns(
  ...
  return DUPLICATE_PATTERNS.filter((pattern) =>
    patterns.has(pattern.patternType),
  );
}
...
export function requiresEscalation(
  ...
  if (riskLevel === "HIGH" && confidence >= 0.9) {
    return true;
  }

  if (duplicateCount >= 3 && confidence >= 0.8) {
    return true;
  }
