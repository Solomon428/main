...
export function generateInputHash(input: DuplicateCheckInput): string {
  ...
  const normalizedData = {
    ...
    invoiceDate: input.invoiceDate ? new Date(input.invoiceDate).toISOString().split('T')[0] : null,
    supplierVAT: input.supplierVAT ? input.supplierVAT.toUpperCase().replace(/[^A-Z0-9]/g, '') : null,
    poNumber: input.poNumber ? normalizePONumber(input.poNumber) : null,
    lineItems: input.lineItems ? input.lineItems.map(item => ({
  ...
  hash.update(JSON.stringify(normalizedData, Object.keys(normalizedData).sort()));
...
export function generatePerceptualHash(input: DuplicateCheckInput): string {
  const normalized = [
    ...
    input.invoiceDate ? new Date(input.invoiceDate).getFullYear().toString() : '0000'
  ].join('');
