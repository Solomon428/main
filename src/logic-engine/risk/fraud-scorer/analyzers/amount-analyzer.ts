...
export function calculateAmountRisk(
  ...
  if (input.totalAmount >= SA_COMPLIANCE_RULES.HIGH_VALUE_THRESHOLD) {
    const deduction = Math.min(30, (input.totalAmount - SA_COMPLIANCE_RULES.HIGH_VALUE_THRESHOLD) / 10000);
    baseScore += deduction;
    riskFactors.push({
  ...
  if (fractionalPart === 0 || fractionalPart > SA_COMPLIANCE_RULES.ROUND_AMOUNT_THRESHOLD) {
    baseScore += 15;
    riskFactors.push({
      category: 'AMOUNT_ANOMALY',
