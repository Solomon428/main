...
export class AnomalyDetector {
  ...
  static async detectAmountOutliers(
    ...
    const isIQRAnomaly = currentAmount < lowerBound || currentAmount > upperBound;
    ...
    if (isZScoreAnomaly || isIQRAnomaly) {
      ...
      return {
        ...
        type: 'AMOUNT_OUTLIER',
        severity,
        details: `Amount R${currentAmount.toLocaleString()} is ${zScore.toFixed(2)} standard deviations ` +
  ...
  static async detectFrequencyAnomaly(
    ...
    if (ratio > 3) {
      return {
        ...
        details: `${recentCount} invoices in ${lookbackDays} days ` +
   ...
   */
  static async detectBehaviorChange(supplierId: string): Promise<AnomalyResult> {
    const now = new Date();
    ...
    if (change > 0.5) {
      ...
      return {
        ...
        details: `Average invoice amount ${direction}d by ${(change * 100).toFixed(1)}% ` +
  ...
  static async runFullAnalysis(
    ...
    if (invoice.supplierId) {
      ...
      // Frequency anomaly
      const frequencyAnomaly = await this.detectFrequencyAnomaly(invoice.supplierId);
      if (frequencyAnomaly.isAnomaly) anomalies.push(frequencyAnomaly);

      // Behavior change
      const behaviorAnomaly = await this.detectBehaviorChange(invoice.supplierId);
