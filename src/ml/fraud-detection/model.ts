...
export class FraudDetectionModel {
  ...
  async analyze(input: FraudDetectionInput): Promise<FraudDetectionOutput> {
    ...
    const amountScore = this.detectAmountAnomaly(input.amount, input.historicalInvoices);
    if (amountScore > 0) {
      riskFactors.push({
        name: 'AMOUNT_ANOMALY',
        score: amountScore,
        weight: this.RISK_WEIGHTS.AMOUNT_ANOMALY,
        description: 'Invoice amount deviates significantly from historical patterns',
      });
      totalScore += amountScore * this.RISK_WEIGHTS.AMOUNT_ANOMALY;
    }

    // 3. Duplicate Pattern Detection
    const duplicateScore = this.detectDuplicatePattern(input.amount, input.invoiceDate, input.historicalInvoices);
    if (duplicateScore > 0) {
      riskFactors.push({
    ...
    const extractionScore = this.assessExtractionConfidence(input.extractionConfidence);
    ...
    const benfordScore = this.analyzeBenfordLaw(input.lineItems.map(li => li.total));
    if (benfordScore > 0) {
      riskFactors.push({
        name: 'BENFORD_LAW',
        score: benfordScore,
  ...
  private detectAmountAnomaly(amount: number, historical: Array<{ amount: number }>): number {
  ...
  private analyzeBenfordLaw(amounts: number[]): number {
    ...
    const deviation = observed.reduce((sum, count, i) => {
  ...
  private calculateConfidence(riskFactors: FraudDetectionOutput['riskFactors']): number {
    if (riskFactors.length === 0) return 0.8;
    return Math.min(0.5 + riskFactors.length * 0.1, 0.95);
  }

  private generateExplanation(score: number, riskFactors: FraudDetectionOutput['riskFactors']): string {
    if (score === 0) {
