...
async function runVATCheck(invoice: any) {
  const vatValid = invoice.vatAmount === (parseFloat(invoice.subtotalExclVAT.toString()) * (parseFloat(invoice.vatRate.toString()) / 100));
...
async function runSupplierCheck(invoice: any) {
  const supplierValid = invoice.supplier?.isVerified && !invoice.supplier?.isBlacklisted;
