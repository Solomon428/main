...
import type { OcrConfig, OcrResult, OcrProgress, LanguageInfo, OcrServiceStatus, ActiveProcess, ExtractionLogData } from './types';
import { OcrServiceError, OcrProcessingError } from './errors';
import { DEFAULT_OCR_CONFIG, METRIC_NAMES, MAX_ACTIVE_JOBS_HEALTHY, ALLOWED_EXTENSIONS, TEMP_FILE_MAX_AGE_MS } from './constants';
...
export class OcrService {
    ...
    try {
      ...
      switch (this.config.provider) {
        case 'tesseract':
          ...
          break;
        case 'google-vision':
          this.googleVisionEngine = new GoogleVisionEngine({ language: this.config.language });
          await this.googleVisionEngine.initialize();
          break;
        case 'aws-textract':
          this.awsTextractEngine = new AwsTextractEngine({ language: this.config.language });
          await this.awsTextractEngine.initialize();
          break;
        case 'azure':
          this.azureEngine = new AzureCognitiveEngine({ language: this.config.language });
          await this.azureEngine.initialize();
          break;
        case 'ollama':
          this.ollamaEngine = new OllamaEngine({ language: this.config.language });
  ...
  async extractTextFromFile(
    ...
    try {
      this.logger.info('Starting text extraction from file', {
        jobId, fileName, mimeType, fileSize: fileBuffer.length, provider: this.config.provider
      });
      ...
      this.updateProgress(jobId, {
        stage: 'initializing', progress: 5, message: 'Initializing OCR engine', timestamp: new Date()
      });

      const result = await this.fileProcessor.processFile(fileBuffer, fileName, mimeType, jobId);
      ...
      if (!result.success || result.confidence < this.config.confidenceThreshold) {
        this.logger.warn('Low confidence OCR result', {
          jobId, confidence: result.confidence, threshold: this.config.confidenceThreshold
        });
      ...
      this.metrics.incrementCounter(METRIC_NAMES.EXTRACTIONS_COMPLETED, 1, {
        provider: this.config.provider, success: result.success, pages: result.pages, language: result.language
      });
      this.metrics.recordHistogram(METRIC_NAMES.PROCESSING_TIME, processingTime, {
        provider: this.config.provider, pages: result.pages
      });
      this.metrics.recordHistogram(METRIC_NAMES.CONFIDENCE_SCORE, result.confidence, { provider: this.config.provider });

      this.updateProgress(jobId, {
        stage: 'completed', progress: 100,
        message: `Text extraction completed: ${result.pages} page(s), ${result.confidence.toFixed(2)}% confidence`,
        details: { pages: result.pages, confidence: result.confidence, processingTime, textLength: result.text.length },
        timestamp: new Date()
      });

      this.logger.info('Text extraction completed successfully', {
        jobId, fileName, pages: result.pages, confidence: result.confidence, processingTime
      });
      ...
      this.logger.error('Failed to extract text from file', {
        jobId, fileName, mimeType, processingTime,
        error: error instanceof Error ? error.message : 'Unknown error'
      });
      ...
      this.metrics.incrementCounter(METRIC_NAMES.EXTRACTIONS_FAILED, 1, {
        provider: this.config.provider, errorType: error instanceof OcrServiceError ? error.code : 'UNKNOWN'
      });

      if (error instanceof OcrServiceError) throw error;

      throw new OcrProcessingError(
        'Failed to extract text from file', 'EXTRACTION_FAILED',
        { fileName, mimeType, processingTime }, error instanceof Error ? error : undefined
      );
    } finally {
  ...
  async extractTextFromUrl(
    ...
    try {
      ...
      this.updateProgress(jobId, {
        stage: 'initializing', progress: 5, message: 'Downloading file from URL', timestamp: new Date()
      });

      const response = await fetch(fileUrl);
      if (!response.ok) {
        throw new OcrServiceError(`Failed to download file: ${response.statusText}`, 'DOWNLOAD_FAILED', {
          url: fileUrl, status: response.status, statusText: response.statusText
        });
      }
      ...
      this.updateProgress(jobId, {
        stage: 'preprocessing', progress: 30, message: 'File downloaded, starting OCR processing',
        details: { fileSize: fileBuffer.length, contentType, fileName }, timestamp: new Date()
      });

      const result = await this.extractTextFromFile(fileBuffer, fileName, contentType, jobId, progressCallback);

      this.logger.info('URL text extraction completed', {
        jobId, fileUrl, totalTime: Date.now() - startTime, confidence: result.confidence
      });
      ...
      this.logger.error('Failed to extract text from URL', {
        jobId, fileUrl, processingTime, error: error instanceof Error ? error.message : 'Unknown error'
      });
      ...
      throw new OcrProcessingError(
        'Failed to extract text from URL', 'URL_EXTRACTION_FAILED',
        { fileUrl, processingTime }, error instanceof Error ? error : undefined
      );
    } finally {
      this.unregisterProcess(jobId);
    }
  ...
  async validateExtractionQuality(
    ...
    return TextPostprocessor.validateExtractionQuality(extractedText, expectedPatterns);
  }

  async getSupportedLanguages(): Promise<LanguageInfo[]> {
    try {
      switch (this.config.provider) {
        case 'tesseract': return this.tesseractEngine?.getLanguages() || [];
        case 'google-vision': return this.googleVisionEngine?.getLanguages() || [];
        case 'aws-textract': return this.awsTextractEngine?.getLanguages() || [];
        case 'azure': return this.azureEngine?.getLanguages() || [];
        case 'ollama': return this.ollamaEngine?.getLanguages() || [];
        default: return [];
      }
    } catch (error) {
      this.logger.error('Failed to get supported languages', {
        provider: this.config.provider, error: error instanceof Error ? error.message : 'Unknown error'
      });
      return [];
    }
  }
  ...
  async getServiceStatus(): Promise<OcrServiceStatus> {
    try {
      ...
      const successfulExtractions = await prisma.ocrExtractionLog.count({
        where: { success: true, createdAt: { gte: new Date(Date.now() - 24 * 60 * 60 * 1000) } }
      });
      ...
      const initialized = this.tesseractEngine !== null || this.googleVisionEngine !== null || 
                         this.awsTextractEngine !== null || this.azureEngine !== null || this.ollamaEngine !== null;

      return {
        initialized, provider: this.config.provider, language: this.config.language,
        activeJobs: this.activeProcesses.size,
        health: this.activeProcesses.size < MAX_ACTIVE_JOBS_HEALTHY ? 'healthy' : 'degraded',
        metrics: {
          totalExtractions: extractionStats._count.id || 0, successfulExtractions, failedExtractions,
          averageProcessingTime: extractionStats._avg.processingTime || 0,
          averageConfidence: extractionStats._avg.confidence || 0
        },
        limits: {
          maxFileSize: this.config.maxFileSize, timeout: this.config.timeout,
      ...
      return {
        initialized: false, provider: this.config.provider, language: this.config.language,
        activeJobs: this.activeProcesses.size, health: 'unhealthy',
        metrics: { totalExtractions: 0, successfulExtractions: 0, failedExtractions: 0,
                   averageProcessingTime: 0, averageConfidence: 0 },
        limits: { maxFileSize: this.config.maxFileSize, timeout: this.config.timeout,
                  allowedMimeTypes: this.config.allowedMimeTypes }
      };
    }
  }
  ...
  private ensureTempDirectory(): void {
    try {
      ...
      this.logger.error('Failed to create temp directory', {
        path: this.tempDir, error: error instanceof Error ? error.message : 'Unknown error'
      });
      throw new OcrServiceError(
        'Failed to create temp directory', 'TEMP_DIR_CREATION_FAILED',
        { path: this.tempDir }, error instanceof Error ? error : undefined
      );
    }
  }
  ...
  private async validateFileInput(fileBuffer: Buffer, fileName: string, mimeType: string): Promise<void> {
    const errors: string[] = [];
    ...
    if (mimeType === 'application/pdf') {
      try {
        ...
        if (!header.startsWith('%PDF-')) errors.push('File does not appear to be a valid PDF');
      } catch { errors.push('Failed to validate PDF file'); }
    }

    if (errors.length > 0) {
      throw new OcrProcessingError('File validation failed', 'VALIDATION_ERROR', {
        fileName, mimeType, fileSize: fileBuffer.length, errors
      });
    }
  }

  private registerProcess(jobId: string, progressCallback?: (progress: OcrProgress) => void): void {
    const timeout = setTimeout(() => {
      this.logger.error('OCR process timeout', { jobId, timeout: this.config.timeout });
      this.unregisterProcess(jobId);
    }, this.config.timeout);

    this.activeProcesses.set(jobId, { jobId, startTime: new Date(), progressCallback, timeout });
    this.metrics.setGauge(METRIC_NAMES.ACTIVE_PROCESSES, this.activeProcesses.size);
  }
  ...
  private updateProgress(jobId: string, progress: OcrProgress): void {
    ...
    if (process?.progressCallback) {
      try { process.progressCallback(progress); }
      catch (error) { this.logger.error('Failed to call progress callback', { jobId }); }
    }
  }
}
