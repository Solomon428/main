...
export class TextPostprocessor {
  ...
  static validateExtractionQuality(
    ...
    const patternMatchScore = expectedPatterns.length > 0 
      ? (matches.filter(m => m.found).length / expectedPatterns.length) * 30
      : 30;
    score += patternMatchScore;
    
    if (nonAlphaNumericRatio < QUALITY_THRESHOLDS.MAX_NON_ALPHANUMERIC_RATIO) score += 20;
    if (averageWordLength >= QUALITY_THRESHOLDS.IDEAL_WORD_LENGTH_MIN && 
        averageWordLength <= QUALITY_THRESHOLDS.IDEAL_WORD_LENGTH_MAX) score += 20;
