...
export class ApproverRouter {
  private static readonly DEFAULT_ROUTING_STRATEGY: RoutingStrategy = 'AMOUNT_BASED';
  ...
  private static readonly SA_APPROVAL_LIMITS: SAApprovalLimit[] = [
    ...
    { role: 'GROUP_FINANCIAL_MANAGER', limit: 5000000, escalationLevel: 'LEVEL_5' } // R5,000,000
  ];
  ...
  private static readonly DEPARTMENT_APPROVAL_LIMITS: DepartmentApprovalLimit[] = [
    ...
    { department: 'PROCUREMENT', baseLimit: 100000, escalationThreshold: 0.7 },
    { department: 'LEGAL', baseLimit: 200000, escalationThreshold: 0.6 },
    { department: 'COMPLIANCE', baseLimit: 150000, escalationThreshold: 0.75 },
  ...
  static async generateApprovalRouting(
    ...
    try {
      ...
      auditTrail.push(this.createAuditEntry('ROUTING_INITIALIZED', routingId, { input, strategy }));
      this.validateInput(input, routingId);
      
      // Step 2: Determine routing strategy
      auditTrail.push(this.createAuditEntry('STRATEGY_DETERMINATION_STARTED', routingId));
      const effectiveStrategy = strategy || this.DEFAULT_ROUTING_STRATEGY;
      auditTrail.push(this.createAuditEntry('STRATEGY_DETERMINATION_COMPLETED', routingId, { effectiveStrategy }));
      
      // Step 3: Calculate effective approval limit based on multiple factors
      auditTrail.push(this.createAuditEntry('APPROVAL_LIMIT_CALCULATION_STARTED', routingId));
      const approvalLimit = this.calculateApprovalLimit(input, routingId);
      auditTrail.push(this.createAuditEntry('APPROVAL_LIMIT_CALCULATION_COMPLETED', routingId, { approvalLimit }));
      
      // Step 4: Apply routing rules to determine escalation level
      auditTrail.push(this.createAuditEntry('RULE_EVALUATION_STARTED', routingId));
      const escalationLevel = this.evaluateRoutingRules(input, approvalLimit, routingId);
      auditTrail.push(this.createAuditEntry('RULE_EVALUATION_COMPLETED', routingId, { escalationLevel }));
      
      // Step 5: Determine total stages based on escalation level and amount
      auditTrail.push(this.createAuditEntry('STAGE_DETERMINATION_STARTED', routingId));
      const totalStages = this.determineTotalStages(input, escalationLevel, routingId);
      auditTrail.push(this.createAuditEntry('STAGE_DETERMINATION_COMPLETED', routingId, { totalStages }));
      
      // Step 6: Generate approval chain with approver assignments
      auditTrail.push(this.createAuditEntry('APPROVAL_CHAIN_GENERATION_STARTED', routingId));
      ...
      auditTrail.push(this.createAuditEntry('APPROVAL_CHAIN_GENERATION_COMPLETED', routingId, { stageCount: approvalChain.length }));
      
      // Step 7: Calculate workload distribution across approvers
      auditTrail.push(this.createAuditEntry('WORKLOAD_DISTRIBUTION_STARTED', routingId));
      const workloadDistribution = this.calculateWorkloadDistribution(approvalChain, routingId);
      auditTrail.push(this.createAuditEntry('WORKLOAD_DISTRIBUTION_COMPLETED', routingId, { workloadDistribution }));
      
      // Step 8: Calculate SLA deadlines for each stage
      auditTrail.push(this.createAuditEntry('SLA_CALCULATION_STARTED', routingId));
      const slaDeadlines = await this.calculateSLADeadlines(approvalChain, input, routingId);
      auditTrail.push(this.createAuditEntry('SLA_CALCULATION_COMPLETED', routingId, { slaDeadlines }));
   ...
   */
  private static validateInput(input: ApprovalRoutingInput, routingId: string): void {
    if (!input.invoiceId || input.invoiceId.trim().length === 0) {
      throw new RoutingException('MISSING_INVOICE_ID', 'Invoice ID is required for approval routing', routingId, 'CRITICAL', 'BLOCK');
    }
    
    if (!input.totalAmount || input.totalAmount <= 0) {
      throw new RoutingException('INVALID_TOTAL_AMOUNT', 'Total amount must be greater than zero', routingId, 'CRITICAL', 'BLOCK');
    }
    
    if (!input.supplierName || input.supplierName.trim().length === 0) {
      throw new RoutingException('MISSING_SUPPLIER_NAME', 'Supplier name is required for approval routing', routingId, 'HIGH', 'BLOCK');
    }
    
    if (!input.department || input.department.trim().length === 0) {
      throw new RoutingException('MISSING_DEPARTMENT', 'Department is required for approval routing', routingId, 'HIGH', 'BLOCK');
    }
    
    if (input.totalAmount > this.MAX_APPROVAL_LIMIT) {
      throw new RoutingException('EXCEEDS_MAX_LIMIT', `Total amount exceeds maximum approval limit of R${this.MAX_APPROVAL_LIMIT.toLocaleString()}`, routingId, 'CRITICAL', 'BLOCK');
    }
  }
  ...
  private static calculateApprovalLimit(
    ...
    const roleMultiplier = this.getRoleMultiplier(input.requesterRole || 'CREDIT_CLERK');
    baseLimit *= roleMultiplier;
    
    // Apply supplier category multiplier
    const supplierMultiplier = this.getSupplierCategoryMultiplier(input.supplierCategory || 'STANDARD');
    baseLimit *= supplierMultiplier;
    
    // Apply risk level multiplier
    const riskMultiplier = this.getRiskLevelMultiplier(input.riskLevel || 'MEDIUM');
    baseLimit *= riskMultiplier;
    
    // Ensure limit is within bounds
    baseLimit = Math.min(this.MAX_APPROVAL_LIMIT, Math.max(this.MIN_APPROVAL_LIMIT, baseLimit));
  ...
  private static getDepartmentBaseLimit(department: string): number {
    const deptLimit = this.DEPARTMENT_APPROVAL_LIMITS.find(d => d.department === department);
  ...
  private static getSupplierCategoryMultiplier(category: string): number {
    const catLimit = this.SUPPLIER_CATEGORY_APPROVAL_LIMITS.find(c => c.category === category);
  ...
  private static getRiskLevelMultiplier(riskLevel: string): number {
    const riskLimit = this.RISK_LEVEL_APPROVAL_LIMITS.find(r => r.riskLevel === riskLevel);
  ...
  private static evaluateRoutingRules(
    ...
    if (input.riskLevel === 'CRITICAL' || input.riskLevel === 'SEVERE') return 'LEVEL_4';
  ...
  private static determineTotalStages(
    ...
    if (input.riskLevel === 'HIGH' || input.riskLevel === 'CRITICAL' || input.riskLevel === 'SEVERE') {
      stages += 1;
    }
  ...
  private static async generateApprovalChain(
    ...
    for (let stage = 1; stage <= totalStages; stage++) {
      ...
      const role = this.determineStageRole(stage, escalationLevel, input.department, routingId);
      
      // Find available approvers for this role and department
      const availableApprovers = await this.findAvailableApprovers(role, input.department, routingId);
      ...
      const selectedApprover = WorkloadBalancer.selectApprover(availableApprovers, routingId);
      ...
      const slaHours = SLACalculator.calculateSLAHours(stage, escalationLevel, input.priority, routingId);
      ...
      chain.push({
        ...
        minAmount: stage === 1 ? 0 : (approvalLimit * (stage - 1) / totalStages),
        maxAmount: stage === totalStages ? Infinity : (approvalLimit * stage / totalStages),
        conditions: this.generateStageConditions(stage, escalationLevel, input, routingId),
        slaHours,
        isOptional: false,
        isParallel: false,
        ...
        backupApprover,
        delegationChain: [],
        escalationPath: this.generateEscalationPath(stage, escalationLevel, routingId),
        assignedDate: new Date(),
        slaDeadline,
        canDelegate: stage < totalStages,
        canEscalate: true,
        requiresComment: stage === totalStages || input.riskLevel === 'HIGH' || input.riskLevel === 'CRITICAL'
      });
    }
  ...
  private static generateStageConditions(
    ...
    conditions.push({
      ...
      operator: 'BETWEEN',
      value: {
        min: stage === 1 ? 0 : (input.totalAmount * (stage - 1) / input.totalStages),
        max: stage === input.totalStages ? Infinity : (input.totalAmount * stage / input.totalStages)
      },
      description: `Amount must be between stage ${stage - 1} and stage ${stage} limits`
    });
    
    // Risk condition for final stage
    if (stage === input.totalStages && (input.riskLevel === 'HIGH' || input.riskLevel === 'CRITICAL')) {
      conditions.push({
        type: 'RISK_THRESHOLD',
        field: 'riskLevel',
        operator: 'IN',
        value: ['HIGH', 'CRITICAL', 'SEVERE'],
        description: 'High risk invoices require additional scrutiny at final stage'
      });
    }
  ...
  private static calculateWorkloadDistribution(
    ...
    const approverMap = new Map<string, { approver: Approver; stages: number; workload: number }>();
    ...
    approverMap.forEach((data, approverId) => {
      distribution.push({
        ...
        capacity: ((data.approver.maxWorkload || 50) - data.workload) / (data.approver.maxWorkload || 50),
  ...
  private static async calculateSLADeadlines(
    ...
    approvalChain.forEach(entry => {
      const slaDeadline = new Date(currentTime + (entry.slaHours * 60 * 60 * 1000));
      deadlines.push({
        stage: entry.stage,
...
export type RoutingErrorAction = 'WARN' | 'BLOCK' | 'ESCALATE' | 'MANUAL_REVIEW';
